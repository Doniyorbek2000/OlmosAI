# sf3d worker — model card

- **Provider ID:** `sf3d`
- **Role:** Fast image→3D with UV unwrap
...
```bash
docker build -f services/ai-workers/sf3d/Dockerfile -t veyra/sf3d:cpu .
docker run -p 8004:8004 --env-file .env veyra/sf3d:cpu
```
